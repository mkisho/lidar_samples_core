...
def listener():
    ...
    rospy.init_node('detec_vert', anonymous=True)

    rospy.Subscriber("/uav1/rplidar_vertical/scan", LaserScan, callback)

    rospy.spin()

def talker():
    pub = rospy.Publisher('/uav1/vert', Bool, queue_size=1)
    pubTorre = isTorre
