...
dataset = pd.read_csv('/home/mathias/catkin_ws/src/lidar_samples_reloaded/datasets/poseHoriz.csv')
X = dataset.iloc[:, 2: 722].values
y = dataset.iloc[:, :2].values
Y1=y[:, 0]
X1=y[:, 1]

y=np.array([math.degrees(math.atan2(X1[i],Y1[i])) for i in range(len(Y1))])
...
X_train= np.reshape(X_train, (3999, 720, 1, 1))
X_test= np.reshape(X_test, (1000, 720, 1, 1))
